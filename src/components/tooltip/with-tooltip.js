  ...
  class WithToolTip extends React.Component {
    ...
    handleMouseOver({event, updateTooltip}) {
      ...
      if (this.state.shouldHaveDynamicPositioning) {
        ...
        absolutePositionOfElementX =
          event.target.getBoundingClientRect().x + windowPageXOffset

        // handles the case wherein the hovered elements is located to the extreme right edge of the screen
        if (
          (absolutePositionOfElementX / windowInnerWidth) * 100 >
            65 ||
      ...
      let actuaXPositioning =
        idealXPositioning + this.contentWidthMeasurerRef.offsetWidth
      ...
      let finalOffsetLeft = shouldHaveDynamicPositioning
        ? idealXPositioning - adjustedXPosOffset - windowPageXOffset
        : updatedMousePosX
      ...
      if (tooltipOptions.customComponent) {
        updateTooltip({
          ...
          type: shouldHaveDynamicPositioning
            ? 'bottom-right'
            : 'default'
        })
      } else {
    ...
    checkShouldHaveTooltip() {
      ...
      if (
        ...
        this.contentWidthMeasurerRef.parentNode.offsetWidth + 5 >
          this.contentWidthMeasurerRef.offsetWidth
      ) {
        this.setState({
          shouldHaveTooltip: false
    ...
    componentDidMount() {
      ...
      if (
        ...
        throw new Error(
          'You must pass one of the options customComponent OR lineContent while calling withTooltip'
        )
      }
    ...
    getRenderContent = () => {
      ...
      if (
        tooltipOptions &&
        tooltipOptions.customComponent &&
        typeof tooltipOptions.customComponent === 'function'
      ) {
      ...
      if (
        ...
        (ComposedComponent &&
          typeof ComposedComponent.toString() === 'string')
      ) {
